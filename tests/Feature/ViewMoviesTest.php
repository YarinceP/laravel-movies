...
class ViewMoviesTest extends TestCase
{
    ...
    public function the_movie_page_shows_the_correct_info()
    {
        ...
        //Release date
        $response->assertSee('2019-12-04');
        //Genres
        $response->assertSee('Action, Adventure, Comedy, Fantasy');
        //Sinopsis
        $response->assertSee('As the gang return to Jumanji to rescue one of their own, they discover that nothing is as they expect. The players will have to brave parts unknown and unexplored.');
        //Crew section
        $response->assertSee('Jeanne McCarthy');
        $response->assertSee('Casting Director');
        //Button play trailer
        $response->assertSee('Play Trailer');
        //Cast section
        $response->assertSee('Dwayne Johnson');
        $response->assertSee('Dr. Smolder Bravestone');
        //Images section
    ...
    {
        $response = Http::fake([
            'https://api.themoviedb.org/3/movie?query=jumanji' => $this->fakeSearchMovies(),
        ]);
     ...
     * @return PromiseInterface
     */
    public function fakeSingleMovie(): PromiseInterface
    {
        return Http::response([
            "adult" => false,
     ...
     * @return PromiseInterface
     */
    private function fakeSearchMovies(): PromiseInterface
    {
        return Http::response([
            'results' => [
                [
